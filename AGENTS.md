...
- Intent recovery closes the pruning loop: the prompt always carries a compact one-line tool index (`with_tool_index`), and `detect_unsent_tool_intent` loads the full schema mid-loop when the model names a pruned tool (bounded by `MAX_TOOL_INTENT_RECOVERIES = 2`). Awareness without ability is useless; this supplies the ability.
